...
<head>
    ...
    <title>Add Confirmation Certificate - DOT My Sacrament</title>
...
<body class="font-['Inter'] bg-gray-100 flex flex-col h-screen">
        ...
        <main class="flex-1 p-6 overflow-y-auto h-full">
            <div class="container mx-auto max-w-7xl">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-semibold text-gray-800">Confirmation Certificates</h1>
                    <button onclick="openAddModal()" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded-lg font-medium transition-colors flex items-center gap-2">
                ...
                <!-- Search Bar -->
                <div class="mb-6">
                    <form method="GET" action="{{ route('subadmin.certificates.add', ['type' => 'confirmation']) }}" class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, certificate ID, or parish..." 
                                   class="w-full px-4 py-3 pl-10 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                            <i class="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                        </div>
                        <button type="submit" class="px-4 py-3 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors">
                            <i class="fas fa-search"></i>
                        </button>
                        @if(request('search'))
                            <a href="{{ route('subadmin.certificates.add', ['type' => 'confirmation']) }}" class="px-4 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                            ...
                                            <p>No confirmation certificates found.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    @foreach ($certificates->getUrlRange(1, $certificates->lastPage()) as $page => $url)
                                        @if ($page == $certificates->currentPage())
                                            <span class="px-3 py-2 text-sm text-white bg-purple-600 rounded-lg">{{ $page }}</span>
                                        @else
    ...
    <div id="certificateModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                <h2 id="modalTitle" class="text-xl font-semibold text-gray-800">Add Confirmation Certificate</h2>
                <button onclick="closeModal()" class="text-gray-400 hover:text-gray-600 text-2xl">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            
            <form id="certificateForm" method="POST" action="{{ route('subadmin.certificates.store', ['type' => 'confirmation']) }}" class="p-6">
                @csrf
                ...
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-primary mb-4">Parent Information</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="place_of_baptism" class="block text-sm font-medium text-gray-700 mb-1">Place of Baptism *</label>
                            <input type="text" id="place_of_baptism" name="place_of_baptism" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-primary mb-4">Confirmation Details</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="date_of_confirmation" class="block text-sm font-medium text-gray-700 mb-1">Date of Confirmation *</label>
                            <input type="date" id="date_of_confirmation" name="date_of_confirmation" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        </div>
                        <div>
                            <label for="place_of_confirmation" class="block text-sm font-medium text-gray-700 mb-1">Place of Confirmation *</label>
                            <input type="text" id="place_of_confirmation" name="place_of_confirmation" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-primary mb-4">Sponsor Information</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                ...
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-primary mb-4">Parish Information</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors">
    ...
    <script>
        ...
        function openAddModal() {
            document.getElementById('modalTitle').textContent = 'Add Confirmation Certificate';
            document.getElementById('certificateForm').reset();
            document.getElementById('certificateId').value = '';
            document.getElementById('formMethod').value = 'POST';
            document.getElementById('certificateForm').action = "{{ route('subadmin.certificates.store', ['type' => 'confirmation']) }}";
            ...
            const content = `
                <div class="space-y-6">
                    ...
                    <div class="bg-green-50 p-4 rounded-lg">
                        <h3 class="font-semibold text-green-900 mb-2">Parent Information</h3>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div class="bg-yellow-50 p-4 rounded-lg">
                        <h3 class="font-semibold text-yellow-900 mb-2">Baptismal Information</h3>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div><span class="text-gray-600">Date of Baptism:</span> <span class="font-medium">${formatDate(certificate.date_of_baptism)}</span></div>
                            <div><span class="text-gray-600">Place of Baptism:</span> <span class="font-medium">${certificate.place_of_baptism || 'N/A'}</span></div>
                        </div>
                    </div>
                    
                    <div class="bg-purple-50 p-4 rounded-lg">
                        <h3 class="font-semibold text-purple-900 mb-2">Confirmation Details</h3>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div><span class="text-gray-600">Date of Confirmation:</span> <span class="font-medium">${formatDate(certificate.date_of_confirmation)}</span></div>
                            <div><span class="text-gray-600">Place of Confirmation:</span> <span class="font-medium">${certificate.place_of_confirmation || 'N/A'}</span></div>
                        </div>
                    </div>
                    
                    <div class="bg-indigo-50 p-4 rounded-lg">
                        <h3 class="font-semibold text-indigo-900 mb-2">Sponsor Information</h3>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div class="bg-green-50 p-4 rounded-lg">
                        <h3 class="font-semibold text-green-900 mb-2">Additional Information</h3>
                        <div class="text-sm">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <h3 class="font-semibold text-gray-900 mb-2">Parish Information</h3>
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div><span class="text-gray-600">Officiant:</span> <span class="font-medium">${certificate.officiant || 'N/A'}</span></div>
        ...
        function editCertificate(certificate) {
            document.getElementById('modalTitle').textContent = 'Edit Confirmation Certificate';
            ...
            document.getElementById('place_of_baptism').value = certificate.place_of_baptism || '';
            document.getElementById('date_of_confirmation').value = certificate.date_of_confirmation || '';
            document.getElementById('place_of_confirmation').value = certificate.place_of_confirmation || '';
            document.getElementById('sponsor1').value = certificate.sponsor1 || '';
            document.getElementById('sponsor2').value = certificate.sponsor2 || '';
            document.getElementById('remarks').value = certificate.remarks || '';
            ...
            // No need to set parish value as it is read-only from auth()->user()
            
            document.getElementById('formMethod').value = 'PUT';
            document.getElementById('certificateForm').action = `/subadmin/certificates/update-confirmation/${certificate.id}`;
            document.getElementById('certificateModal').classList.remove('hidden');
        }
        ...
        function deleteCertificate(id, name) {
            if (confirm(`Are you sure you want to delete the certificate for "${name}"? This action cannot be undone.`)) {
                ...
                form.method = 'POST';
                form.action = `/subadmin/certificates/delete-confirmation/${id}`;
        ...
        function openGenerateModal(certificate) {
            ...
            document.getElementById('generateForm').action = `/subadmin/certificates/confirmation/${certificate.id}/download`;
