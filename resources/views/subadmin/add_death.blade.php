...
<head>
    ...
    <title>Add Death Certificate - DOT My Sacrament</title>
...
<body class="font-['Inter'] bg-gray-100 flex flex-col h-screen">
        ...
        <main class="flex-1 p-6 overflow-y-auto h-full">
            <div class="container mx-auto max-w-7xl">
                ...
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-semibold text-gray-800">Death Certificates</h1>
                    <button onclick="openAddModal()" class="bg-gray-700 hover:bg-gray-800 text-white px-6 py-2.5 rounded-lg font-medium transition-colors flex items-center gap-2">
                ...
                <!-- Search Bar -->
                <div class="mb-6">
                    <form method="GET" action="{{ route('subadmin.certificates.add', ['type' => 'death']) }}" class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, certificate ID, or parish..." 
                                   class="w-full px-4 py-3 pl-10 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-gray-500 focus:border-transparent">
                            <i class="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
                        </div>
                        <button type="submit" class="px-4 py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors">
                            <i class="fas fa-search"></i>
                        </button>
                        @if(request('search'))
                            <a href="{{ route('subadmin.certificates.add', ['type' => 'death']) }}" class="px-4 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    @foreach ($certificates->getUrlRange(1, $certificates->lastPage()) as $page => $url)
                                        @if ($page == $certificates->currentPage())
                                            <span class="px-3 py-2 text-sm text-white bg-gray-600 rounded-lg">{{ $page }}</span>
                                        @else
    ...
    <div id="certificateModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                <h2 id="modalTitle" class="text-xl font-semibold text-gray-800">Add Death Certificate</h2>
                <button onclick="closeModal()" class="text-gray-400 hover:text-gray-600 text-2xl">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            
            <form id="certificateForm" method="POST" action="{{ route('subadmin.certificates.store', ['type' => 'death']) }}" class="p-6">
                @csrf
                ...
                <div class="mb-6">
                    <h3 class="text-lg font-semibold text-primary mb-4">Deceased Information</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            <label for="place_of_cemetery" class="block text-sm font-medium text-gray-700 mb-1">Place of Cemetery *</label>
                            <input type="text" id="place_of_cemetery" name="place_of_cemetery" required
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent">
                        </div>
                        <div>
                        ...
                        <div class="md:col-span-2">
                            <label for="residents_address" class="block text-sm font-medium text-gray-700 mb-1">Residents Address</label>
                            <input type="text" id="residents_address" name="residents_address"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="priest_name" class="block text-sm font-medium text-gray-700 mb-1">Minister of Death</label>
                            <select id="priest_name" name="priest_name"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-500 focus:border-transparent">
                                <option value="">Select Minister</option>
                                @foreach($ministers ?? [] as $minister)
                                    <option value="{{ $minister->full_name }}" {{ old('priest_name') == $minister->full_name ? 'selected' : '' }}>
                                        {{ $minister->full_name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-gray-700 hover:bg-gray-800 text-white rounded-lg transition-colors">
    ...
    <script>
        ...
        function openAddModal() {
            document.getElementById('modalTitle').textContent = 'Add Death Certificate';
            document.getElementById('certificateForm').reset();
            document.getElementById('certificateId').value = '';
            document.getElementById('formMethod').value = 'POST';
            document.getElementById('certificateForm').action = "{{ route('subadmin.certificates.store', ['type' => 'death']) }}";
                ...
                <div class="space-y-6">
                    <div class="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div class="col-span-2"><span class="text-gray-600">Place of Cemetery:</span> <span class="font-medium">${certificate.place_of_cemetery || 'N/A'}</span></div>
                            <div><span class="text-gray-600">Father's Full Name:</span> <span class="font-medium">${certificate.fathers_full_name || 'N/A'}</span></div>
                            <div><span class="text-gray-600">Mother's Full Name:</span> <span class="font-medium">${certificate.mothers_full_name || 'N/A'}</span></div>
                            <div class="col-span-2"><span class="text-gray-600">Residents Address:</span> <span class="font-medium">${certificate.residents_address || 'N/A'}</span></div>
                        </div>
                    </div>
        ...
        function editCertificate(certificate) {
            document.getElementById('modalTitle').textContent = 'Edit Death Certificate';
            document.getElementById('certificateId').value = certificate.id;
            document.getElementById('full_name').value = certificate.full_name || '';
            document.getElementById('date_of_death').value = certificate.date_of_death || '';
            document.getElementById('place_of_cemetery').value = certificate.place_of_cemetery || '';
            ...
            document.getElementById('residents_address').value = certificate.residents_address || '';
            ...
            document.getElementById('priest_name').value = certificate.priest_name || '';
            
            document.getElementById('formMethod').value = 'PUT';
            document.getElementById('certificateForm').action = `/subadmin/certificates/update-death/${certificate.id}`;
            document.getElementById('certificateModal').classList.remove('hidden');
        }
        ...
        function deleteCertificate(id, name) {
            if (confirm(`Are you sure you want to delete the certificate for "${name}"? This action cannot be undone.`)) {
                ...
                form.method = 'POST';
                form.action = `/subadmin/certificates/delete-death/${id}`;
        ...
        function openGenerateModal(certificate) {
            ...
            document.getElementById('generateForm').action = `/subadmin/certificates/death/${certificate.id}/download`;
